	...
	const setDefaultValue = (nameField) => {
		if(props.modalType === 'put'){
			const defaultValue = props.contacts.find(item=> item.id === props.contactId),
				firstName = nameField === 'firstName' && defaultValue.name.split(' ')[0],
	...
	return <form onSubmit={handleSubmit(onSubmit)}>
		<div className={s.form__part}>
			<Input
				...
				fieldData={{placeholder: 'Name', type: 'text', id: 'firstName', defaultValue: setDefaultValue('firstName')}}
			/>

			<Input
				name={'lastName'} isLabel={false} useForm={{register, errors}}
				validate={{
					required: 'Field is required',
					maxLength: {value: 15, message: 'Max length is 15'},
					minLength: {value: 3, message: 'Min length is 3'},
				...
				fieldData={{placeholder: 'Surname', type: 'text', id: 'lastName', defaultValue: setDefaultValue('lastName')}}
			/>
		</div>
		...
		<div className={s.form__part}>
			<Input
				...
				fieldData={{placeholder: 'Company', type: 'text', id: 'company', defaultValue: setDefaultValue('company')}}
			/>

			<Input
				name={'job'} isLabel={false} useForm={{register, errors}}
				validate={{
					required: 'Field is required',
					maxLength: {value: 20, message: 'Max length is 20'},
				}}
				fieldData={{placeholder: 'Job', type: 'text', id: 'job', defaultValue: setDefaultValue('job')}}
			/>
		</div>
		...
		<div className={s.form__part}>
			<Input
				...
				fieldData={{placeholder: '+7(910)-221-22-22', type: 'text', id: 'phone',  defaultValue: setDefaultValue('phone')}}
			/>

			<Input
				name={'email'} isLabel={false} useForm={{register, errors}}
				validate={{required: 'Field is required',}}
				fieldData={{placeholder: 'Email', type: 'email', id: 'email', defaultValue: setDefaultValue('email')}}
			/>
		</div>
